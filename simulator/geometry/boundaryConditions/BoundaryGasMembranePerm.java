 ...
 */
public class BoundaryGasMembranePerm extends ConnectedBoundary
{
	/**
	...
	public void refreshBoundary(SoluteGrid aSoluteGrid)
	{
		...
		if ( isPermeableTo[aSoluteGrid.soluteIndex] )
			while (_myShape.followBoundary(dcIn, dcOut, aSoluteGrid))
			{
				...
				double val = aSoluteGrid.getValueAt(dcIn);
				double factor = 0.0001 * (double)dcIn.j / (double)aSoluteGrid.getGridSizeJ();
				
				val = val + factor * permeability[aSoluteGrid.soluteIndex] * (bulkValue - val);
				//if (dcIn.j > 50)
					aSoluteGrid.setValueAt(Math.max(val, 0), dcIn);
				//else
				//	aSoluteGrid.setValueAt(0.0, dcIn);
